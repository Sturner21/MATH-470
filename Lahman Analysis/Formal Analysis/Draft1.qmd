...
The parameter $\delta_p$ (which is $\xi_p$ in the actual paper) is the effect that playing in park $p$ has on the HR probability $\pi_{nip}$. As discussed in @sec-BaseBack, there is strong precedent to include such a term because the variance between ballpark dimension and enviornment can be drastic. Fellingham and Fisher (2017) estimate the effect that each park has on a player's ability to hit a HR and then add the associated effect to the players who played at that ballpark in a given year.
