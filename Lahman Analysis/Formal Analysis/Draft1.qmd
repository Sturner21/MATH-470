...
In explore results, discuss both the trajectories of about 4-5 players and show how they change (probably want to sample about 10 just to be safe). But then, just for fun, look at which player has the highest $\alpha$ level since that would be my models prediction of best HR hitter independent of everything else. It will probably be Barry Bonds.
...
This section should be dedicated to talking about general fit of the model. Explore using the normal plots that we've been using with the confidence intervals to see if the parameters are near their priors. And, also explore using the trace-plots. The linegraphs that go all over the place.
...
See @fig-p582Act and @fig-p582Pred.

## Junior Felix {#sec-felix}
...
See @fig-p177Act and @fig-p177Pred.

Probably don't need to include this examination. Paper will probably be long enough and he's not that interesting.
...
#651, 582, 177, 420
traceplot(fit.poly, pars = c("alpha[651]", "alpha[582]", "alpha[177]", "alpha[420]"))
```
