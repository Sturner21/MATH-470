...
\\
\log(\frac{\pi_{nip}}{1-\pi_{nip}})&=[\beta_{n1}\cdot Age_{ni}]+[\beta_{n2}\cdot(Age_{ni})^2]+[\beta_{n3}\cdot(Age_{ni})^3]+[\beta_{n4}\cdot(Age_{ni})^4]+\alpha_n+\xi_i
+\delta_p
\end{align}
$$ {#eq-FFmodel}
...
This section should be dedicated to talking about general fit of the model. Explore using the normals plots that we've been using with the confidence intervals to see if the parameters are near their priors. And, also explore using the trace-plots. The linegraphs that go all over the place.
...
See @fig-p651Act and @fig-p651Pred.
