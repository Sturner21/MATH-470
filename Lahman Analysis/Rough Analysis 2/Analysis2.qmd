...
We can see the age factor has an almost negligible effect on player performance, at least when we only allow the effect to be linear. Fellingham and Fisher allowed for it to be non-linear so the next steps should be focused on investigating that relationship to see if it adds to our model at all.

### Nonlinear effects of Age

Before diving into the full quadratic that Fellingham and Fisher used, we should first check out what it looks like by simply making it non-linear.
...
data28 <- list(
  ...
  Player = player_index_small %>% pull(player_index),
  cAge = player_index_small %>% pull(cAge),
  cAgeSq = (player_index_small %>% pull(cAge))**2
)
```
...
```{r}
saveRDS(fit28, "HR28.rds")
```

Now we can plot to see what the effect is on the different intercepts.

```{r}
plot(fit28, pars="a")
```

Nothing really has changed here in the plot compared to the other previous ones.
...
```{r}
summary(fit28, 'beta1')$summary
summary(fit28, 'beta2')$summary
```

We can see from these that the beta1 and beta2 are much lower than we expected with our priors. So, we can probably make their priors more confident in the remaining cubic and quadratic terms of the next model.
...
data28b <- list(
  ...
  cAge = player_index_small %>% pull(cAge),
  cAgeSq = (player_index_small %>% pull(cAge))**2,
  cAgeCb = (player_index_small %>% pull(cAge))**3
)
```
...
So that took a super long time and I was still running into the error of log(0), and that was after taking out the quadratic term.

```{r}
summary(fit28b, 'beta1')$summary
summary(fit28b, 'beta2')$summary
summary(fit28b, 'beta3')$summary
```

So, these are huge with big standard errors suggesting that we need more iterations.

Chains aren't mixing even after 2000 iterations, it took forever to run, and the RDS is too big for GitHub to handle so I think we can chalk this up as a failure.

### 11/10 Meeting

```{r}
#Set the seed since the players chosen keep changing
set.seed(300)
...
```{r}
plot(fit28aa, pars="beta0")
```

```{r}
plot(fit28aa, pars="beta1")
```
...
```{r}
plot(fit28ab, pars="beta0")
plot(fit28ab, pars = "beta1")
plot(fit28ab, pars = "beta2")
```

Start plotting individual player's home run "trajectories" to see how the model is doing. Should be a parabola from the means of the posterior. This would show the logit of player home runs. Prefer players who have lots of seasons to start and then do it for players who have very little seasons.

After, consider either more than 100 players or add in other effects like park effects.

### Modeling Player "Trajectories" From Model Prediction

Followed [this vignette](http://mjskay.github.io/tidybayes/articles/tidy-rstanarm.html) from tidybayes.

```{r}
#Start by installing tidybayes

#This shows us what the variables are called in the model (we already know this from the stan code)
get_variables(fit28ab)
```

```{r}
#Grabs the beta1's from the model and shows first 10
fit28ab %>% 
  spread_draws(beta1[group]) %>% 
  head(10)
```

```{r}
#Shows the first 10 beta1's again but also gives medians
fit28ab %>% 
  spread_draws(beta1[group]) %>% 
  median_qi() %>% 
  head(10)
...
```{r}
#Gives us a tidy df of all the outputs from when we print a model fit for a specific parameter
fit28ab %>% 
  spread_draws(beta1[group]) %>% 
  summarise_draws()
```
...
```{r}
#Though ugly, this output should be replicating the beta1 graph above.
fit28ab %>%
  spread_draws(beta1[group]) %>%
  median_qi(.width = c(.95, .8)) %>%
  ggplot(aes(y = group, x = beta1, xmin = .lower, xmax = .upper)) +
  geom_pointinterval()
```

```{r}
#Note: in the vingette, condition=HR for us and ABC=Samp_Batting
Samp_Batting %>%
  modelr::data_grid(HR) %>%
  add_epred_draws(fit28ab) %>%
  head(10)
```
...
I'm getting some errors here, so I'm going to take a break from trying to use tidybayes and give this a go another way.

We talked in the meeting on 11/10 about picking a player who had a lot of seasons so we could see how their trajectory looked over the course of their long career. Selecting arbitrarily, playerID "jenkige01" played from 2003-2013. So a nice 11 year long career. (His index is 46)

```{r}
#Grab player 9's estimates for beta0, beta1, and beta2
b0_i46 <- fit28ab %>% 
  spread_draws(beta0[group]) %>% 
  summarise_draws() %>% 
  filter(group == 46)
...
```{r}
#Combine these into one tibble
player_46_est <- b0_i46 %>% 
  bind_rows(b1_i46, b2_i46)
```

```{r}
#We want to plot this guy's fit compared to how he actually did
player_46_Samp_Batting <- Samp_Batting %>% 
  filter(player_index==46) %>% 
  select(playerID, player_index, yearID, Age, AB, HR) %>% 
  mutate(cAge = Age - 30, .after = Age) %>% 
  mutate(p = HR / AB) %>% 
  mutate(p_logit = log(p / (1-p)))
```

```{r}
#This plot shows us how he actually did with the black points compared to how we would predict him to do, the red line
ggplot(data = player_46_Samp_Batting)+ 
  aes(x = cAge, y=p_logit)+
  geom_point()+
  geom_function(fun = function(x) (-2.967667)	+ (-0.0481313*x) + (-0.00995784*(x**2)), color="red")
```

```{r}
#Extending the x-axis we can see how our model would predict him to do from the ages of 20-40 even though he only played from when he was 24-34.
ggplot(data = player_46_Samp_Batting)+ 
  aes(x = cAge, y=p_logit)+
  geom_point()+
  geom_function(fun = function(x) (-2.967667)	+ (-0.0481313*x) + (-0.00995784*(x**2)), color="red")+
...
It is difficult to tell what the logit scale means, so in terms of probability, -3.5 would be about 3%. So, we would predict him to be an average home run hitter when he entered the league at 20. Then it peaks around -2.85 which is about 5.5%. And then when he's 40 he has an expected home run rate of -4.5 which is about 1.1%.

This outlines the (somewhat laborious) process of how to extrapolate predictions from our model for specific players. Thus, I think the next thing to do would be to add in different factors, like park and year factors since I believe that they would give our model the strongest edge. I will do this on a new document though because this one is getting unwieldy.\
\
\
\
...
At the recommendation of Dr. Parson, I think that it would be really interesting to consider a 10 year sample of MLB seasons rather than just doing to the 2-year one done above. I liked how the HR27d analysis came out so we'll copy that down here.

```{r}
ten_year_batting <- Batting %>% 
  #Consider only two years to get a managable amount of players
  filter(yearID >= 2010 & yearID <= 2019) %>% 
  #Number the players for the multilevel model
  mutate(player_index = as.factor(playerID), .after = playerID) %>% 
  mutate(player_index = as.integer(player_index)) %>% 
  mutate(cAge = Age - 30, .after = Age) %>% 
...
```{r}
data29 <- list(
  N = nrow(ten_year_batting),
  n_year = n_distinct(ten_year_batting$yearID),
  n_park = n_distinct(ten_year_batting$park_index),
  n_player = n_distinct(ten_year_batting$player_index),
  AB = ten_year_batting %>% pull(AB),
  HR = ten_year_batting %>% pull(HR),
  park = ten_year_batting %>% pull(park_index),
  Year = ten_year_batting %>% pull(year_index),
  cAge = ten_year_batting %>% pull(cAge),
  Player = ten_year_batting %>% pull(player_index)
)
```
...
200 iterations took 15 minutes. So, we should wait until we have some time to render this or really try and get this to work on my PC.
