...
I'm getting som errors here, so I'm going to take a break from trying to use tidybayes and give this a go another way.

We takled in the meeting on 11/10 about picking a player who had a lot of seasons so we could see how their trajectory looked over the course of their long career. Selecting arbitrarily, playerID "bayja01" played from 2003-2013. So a nice 11 year long career. (His index is 9)
...
player_9_est <- b0_i9 %>% 
  bind_rows(b1_i9, b2_i9)
```

```{r}
#We want to plot this guy's fit compared to how he actually did
player_9_Samp_Batting <- Samp_Batting %>% 
  filter(player_index==9) %>% 
...
```{r}
ggplot(data = player_9_Samp_Batting)+ 
  aes(x = cAge, y=p_logit)+
  geom_point()+
  geom_function(fun = function(x) (-2.962388)	+ (-0.0443416*x) + (-0.004696772*(x**2)), color="red")
```
