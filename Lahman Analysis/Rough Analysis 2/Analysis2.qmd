...
sum_all_years_SB <- Batting %>% group_by(yearID, Age, SB) %>%
  summarize(HR = sum(HR), AB = sum(AB)) %>% ungroup
```
...
(Here is an RStan demo with some inspirational code: [http://avehtari.github.io/BDA_R_demos/demos_rstan/ppc/poisson-ppc.html])
...
```{r}
ppc_hist(sum_all_years_SB %>% pull(HR), HR_rep[1:8, ], binwidth = 1)
```

Yuck... A poor fit. The model doesn't reproduce low numbers of home runs as we see in the original data set.

```{r}
ppc_dens_overlay(sum_all_years_SB %>% pull(HR), HR_rep[1:50, ])
```
This family of density plots shows the same thing. A basic check of the model (histogram of home-run counts in our groups) shows a mismatch.

The situation reminds me of the pictures in Section 26.2 of the Stan User's Guide [https://mc-stan.org/docs/2_26/stan-users-guide/plotting-multiples.html]. In that section, the authors show an overdispersed count variable (negative binomial) fit with a binomial model. There is more on the same example in Section 26.3, using a test statistic and the p-value idea to quantify the poor fit.
